<?php
/*
Plugin Name: WooCommerce Product Slider Child
Description: A simple WooCommerce product slider plugin.
Author: Nutthanich Somprakit
Version: 1.2
...
class WooCommerce_Product_Slider_child {
	...
	public function wa_wps_settings_link($actions, $file) {
		if(false !== strpos($file, 'woocommerce-product-slider'))
		 $actions['settings'] = '<a href="options-general.php?page=woocommerce-product-slider-child">Settings</a>';
		return $actions; 
	}
	...
	foreach( $myposts as $post ){
		...
		if($display_title){
		$slider_gallery.= '<span class="wps_title"><a  style=" text-decoration:none;" href="'.$post_link.'">'.$post_title.'</a></span>';
		}
